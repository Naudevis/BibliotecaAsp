...
namespace Biblioteca.Business.Repositories
{
    public class BookRepository : IBookRepository
    {
        ...
        public async Task<IEnumerable<Book>> SearchByAuthorBooks(string data)
        {
           
            var query = _context.Books.Include(b => b.Author).Include(s => s.Status).Where(p =>
            p.Author.Author_id ==data  &&
            p.Status_id != 2).OrderBy(b => b.Title).ToListAsync(); // Filtra por código
            if (query == null)
            {
                Console.WriteLine("Hola");
                return await query; // Ejecuta la consulta y retorna el resultado
            }
            else
            {
                return await query; // Ejecuta la consulta y retorna el resultado
        ...
        public async Task<List<Book>> GetBooksMoreOldAsync()
        {
           
            try
            {

                return await _context.Books.Include(b => b.Author).Where(b=>b.Status_id==1).OrderBy(b=>b.CreateDate).Take(10).ToListAsync();
        ...
        public byte[] GeneratePdfReport(List<Book> books)
        {
            ...
            graphics.DrawString("Reporte de los libros registrados más antiguos", ReportTitle, XBrushes.Gray, new XPoint(25, 220));
            ...
            graphics.DrawString("Precio", headerFont, XBrushes.White, new XPoint(340, 248 + 5));
            graphics.DrawString("Fecha de registro", headerFont, XBrushes.White, new XPoint(437, 248 + 5));
            ...
            foreach (var book in books)
            {
                ...
                var authorText = book.CreateDate.ToString();
                var TitleText = book.Title;
                var StockText = book.Price.ToString();
        ...
        public byte[] GeneratePdfReportAuthor(List<Book> books)
        {
            var document = new PdfDocument();
            ...
            if(books.Count>0)
            {
                graphics.DrawString($"Reporte de libros escritos por {books.FirstOrDefault().Author.Name}", ReportTitle, XBrushes.Gray, new XPoint(25, 220));

            }
            else
            {
                graphics.DrawString($"Reporte de libros por autor", ReportTitle, XBrushes.Gray, new XPoint(25, 220));
